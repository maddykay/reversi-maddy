...
/* If we aren't on Heruko, then we need to adjust our port and directory */
...
io.on('connection', (socket) => {
    ...
    */
    ...
    socket.on('invite', (payload) => {
        ...
        /* Make sure that the invited is present */
    ...
    */
    /* send_chat_message_response:
         {
             'result': 'success',
             'username': the user that sent themessage,
    ...
    socket.on('play_token', (payload) => {
        ...
        if (
            ...
            let response = {
                result: 'fail',
                message: 'play_token played the right color, but the wrong player'
            }
            socket.emit('play_token_response', response);
            serverLog('play_token command failed', JSON.stringify(response));
            return;
...
function send_game_update(socket, game_id, message) {
    ...
    if (legal_moves === 0) {
        ...
        if (whitesum > blacksum) {
            winner = "Vanilla sprinkle donut";
        }
        if (whitesum < blacksum) {
            winner = "Chocolate sprinkle donut";
        }
